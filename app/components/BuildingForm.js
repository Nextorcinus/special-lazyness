...
export default function BuildingForm({ category, selectedSub, onCalculate }) {
	...
	const handleSubmit = () => {
		...
		if (result) {
			toast.success(
				`The upgrade calculation was successful with Valeria Lv.${valeriaLevel} (+${bonusPercent}% SvS)`,
			);
			onCalculate(result);
		} else {
